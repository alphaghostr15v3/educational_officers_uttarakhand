...
<div class="container my-5">
    <div class="row g-4">
        ...
        <div class="col-md-12 mt-4">
            ...
            @else
            {{-- Guest: show only PUBLIC work forms --}}
            @if($public_work_forms->count() > 0)
            <div class="portal-grid mb-4">
                @foreach($public_work_forms as $workType => $forms)
                <a href="{{ route('work-forms.by-type', urlencode($workType)) }}" class="portal-grid-item">
                    <div class="icon-wrapper">
                        @php
                            $icons = [
                            ...
                            $icon = $icons[$workType] ?? 'fa-file';
                        @endphp
                        <i class="fas {{ $icon }} fa-2x text-success"></i>
                    </div>
                    <div class="title-en">{{ $workType }}</div>
                    <div class="title-hi text-muted small">{{ $forms->count() }} {{ $forms->count() == 1 ? 'Document' : 'Documents' }}</div>
                </a>
            ...
            {{-- Login prompt to see more --}}
            <div class="d-flex align-items-center gap-3 p-3 rounded-4 border border-primary-subtle bg-primary-subtle mb-5">
                <i class="fas fa-lock fa-2x text-primary opacity-75 flex-shrink-0"></i>
                <div class="flex-grow-1">
                    <div class="fw-bold text-primary">More forms available after login</div>
                    <div class="text-muted small">Login to access all departmental work forms, templates &amp; administrative documents.</div>
                </div>
                <div class="d-flex gap-2 flex-shrink-0">
                    <a href="{{ route('employee.login') }}" class="btn btn-primary btn-sm fw-bold px-3">
                        <i class="fas fa-sign-in-alt me-1"></i> Login
                    </a>
                    <a href="{{ route('employee.register') }}" class="btn btn-outline-primary btn-sm fw-bold px-3">Register</a>
                </div>
            </div>
            @endauth
        </div>
